...
def train_lstm(batch_size=2,time_step=1,train_begin=0,train_end=480):  
    ...
    with tf.Session() as sess:  
        ...
        for i in range(iter_time):  
            for step in range(len(batch_index)-1):
                '''
                #print(np.shape(train_x[batch_index[step]:batch_index[step+1]]))
                #print(np.shape(train_y[batch_index[step]:batch_index[step+1]]))
                #print(np.shape(np.reshape(train_y[batch_index[step]:batch_index[step+1]],[batch_size,1,1])))
'''
    ...
    return test_predict  
test_predict = train_lstm(batch_size=80,time_step=1,train_begin=0,train_end=487) 
